...
<div style='text-align: justify;'>
    ...
    retain valuable customers. Using the XGBoost Classifier, our model analyzes customer
...
def compute_rfm(df):
    ...
    if df.shape[0] == 1:
        ...
        # Menggunakan 3 kategori scoring (range 1-3)
        r_bins = [-1, 7, 14, 31]  # Recency: 3 kategori
        f_bins = [0, 5, 10, 16]  # Frequency: 3 kategori
        m_bins = [-1, 100, 200, 325]  # Monetary: 3 kategori

        # Memberikan skor berdasarkan bins
        df['R_Score'] = pd.cut(df['DaySinceLastOrder'], bins=r_bins, labels=[3, 2, 1], include_lowest=True)
        df['F_Score'] = pd.cut(df['OrderCount'], bins=f_bins, labels=[1, 2, 3], include_lowest=True)
        df['M_Score'] = pd.cut(df['CashbackAmount'], bins=m_bins, labels=[1, 2, 3], include_lowest=True)

        # Konversi skor ke integer
        df[['R_Score', 'F_Score', 'M_Score']] = df[['R_Score', 'F_Score', 'M_Score']].astype(int)

    # Menghitung total RFM Score
    df['RFM_Score'] = df['R_Score'] + df['F_Score'] + df['M_Score']

    # Menentukan Segmen Pelanggan berdasarkan segmentasi baru
...
def create_user_input():
    ...
    CashbackAmount = st.sidebar.number_input('Amount of Cashback (max 324.99)', min_value=0.0, max_value=324.99, value=0.0, step=1.0)
